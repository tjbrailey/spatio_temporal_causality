...
res.frame <- read.table(paste0(here::here(), "/data/resampling_data.txt"), header = TRUE)
...
ggsave(plot = last_plot(), filename = paste0(here::here(), "/figures/teststat_resampled_noconf.pdf"))
...
ggplot(subset(res.frame, b > 0 & model == "popdens"), aes(x=ts)) + geom_histogram(color="gray", alpha=0.7) + 
  ...
  xlab("test statistic") + theme(text = element_text(size=15), plot.title = element_text(hjust=.5))
ggsave(plot = last_plot(), filename = paste0(here::here(), "/figures/teststat_resampled_popdens.pdf"))
...
ggplot(subset(res.frame, b > 0 & model == "roaddist"), aes(x=ts)) + geom_histogram(color="gray", alpha=0.7) + 
  ...
  xlab("test statistic") + theme(text = element_text(size=15), plot.title = element_text(hjust=.5))
ggsave(plot = last_plot(), filename = paste0(here::here(), "/figures/teststat_resampled_roaddist.pdf"))
...
ggplot(subset(res.frame, b > 0 & model == "lscm"), aes(x=ts)) + geom_histogram(color="gray", alpha=0.7) + 
  ...
  xlab("test statistic") + theme(text = element_text(size=15), plot.title = element_text(hjust=.5))
ggsave(plot = last_plot(), filename = paste0(here::here(), "/figures/teststat_resampled_lscm.pdf"))
