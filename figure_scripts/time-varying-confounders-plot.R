...
df.coef <- read.table(paste0(here::here(), "/data/time-var-conf.txt"), header = TRUE)
...
ggsave(plot = last_plot(), filename = paste0(here::here(), "/figures/b0hat.pdf"))

ggplot(df.coef, aes(b1)) + geom_histogram() + 
  facet_grid(method~var, scales = "free_y", labeller = label_parsed) + 
  geom_vline(xintercept=2, col="red") + 
  xlab(expression(paste("estimate of ", beta[1])))
ggsave(plot = last_plot(), filename = paste0(here::here(), "/figures/b1hat.pdf"))
